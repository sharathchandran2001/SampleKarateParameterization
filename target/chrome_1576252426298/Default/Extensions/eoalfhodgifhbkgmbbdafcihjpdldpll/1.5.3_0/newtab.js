...
function ViewRenderer(newTabData, unittest) { //jshint ignore: line
    ...
    //If some argument newTabData misses some information, will set a default value for them
     function populateMissingData(newTabData){
        offlinePhotosIdx =  extGlobal.browserGap.localStorage.getItem("offlinePhotosIdx") || 0;
    ...
    //Render background image;
     function renderBackground() {
        ...
        var photoData =  extGlobal.browserGap.localStorage.getItem("bgPhoto");
    ...
    //display (and link) the owner of the background image
     function renderOwnerData(){
        ...
        if(ownername && owner) {
            ...
            ownerLink.setAttribute("href", "https://flickr.com/photos/"+owner);
    ...
    function toggleTN () {
     ...
     function toggleBN () {
        ...
        var breakingNewsBar =  extGlobal.browserGap.localStorage.getItem("breakingNewsBar");
     ...
     function renderBreakingNews() {
        ...
        if (newTabData.breakingNews && breakingNewsBar !== "hidden") {
            ...
            var bnAlreadyShownS =  extGlobal.browserGap.localStorage.getItem("bnAlreadyShown") || "{}";
     ...
     function renderTrendingNow() {
        ...
        var breakingNewsBar =  extGlobal.browserGap.localStorage.getItem("breakingNewsBar");
        var trendingBar =  extGlobal.browserGap.localStorage.getItem("trendingBar");
        ...
        if (typeof newTabData.trendingNowData === "undefined" || !newTabData.trendingNowData ||
            ...
            extGlobal.constants.mapQuestUI || (newTabData.breakingNews && breakingNewsBar !== "hidden")) {
            return;
        }
    ...
    function sendTnClickBeacon (target) {
     ...
     function sendBnClickBeacon (breakingNews, finalBnUrl) {//Breaking News click beacon
        var beaconConfig = {};
        var beaconParams = {};
        var bnAlreadyShown =  extGlobal.browserGap.localStorage.getItem("bnAlreadyShown") || "{}";
     ...
     function renderWeatherData(){
        ...
        var weatherContainer = document.getElementById("weatherInfo"),
            ...
            localStorageWeather =  extGlobal.browserGap.localStorage.getItem("localStorageWeather"),
            unit =  extGlobal.browserGap.localStorage.getItem("unit");
        ...
        if (localStorageWeather) {
            ...
            /*location added*/
            city.textContent =  extGlobal.browserGap.localStorage.getItem("location");
    ...
    function getPartnerInPosition(partners, position) {
     ...
     function renderNewTopSites(parent, newFav) {
        ...
        var sitesBlackListStr =  extGlobal.browserGap.localStorage.getItem('sitesBlackList') || null;
    ...
    function initFavorites() {
     ...
     function getFavoriteSites() {
        try {
            var favoriteSitesS =  extGlobal.browserGap.localStorage.getItem("favoriteSites") || null;
    ...
    function switchView() {
     ...
     function renderSearchBar() {
        var searchBar =  extGlobal.browserGap.localStorage.getItem("searchBar");
     ...
     function toggleEditMode() {
        ...
        if(editModeToggle) {
            ...
            if (sitesLength === 0) { // when sites are all empty and user clicks on "Edit Favorites" we'll delete the blacklist to bring back all sites
                 extGlobal.browserGap.localStorage.removeItem("sitesBlackList");
                sitesBlackList = [];
                renderNewTopSites(document.querySelector(".newTopSitesContainer"));
            }
            renderEditMode();
    ...
    //render everything
     function render() {
        ...
        renderToggleView();
        if(extGlobal.constants.weatherUI) {
            var localStorageWeather =  extGlobal.browserGap.localStorage.getItem("localStorageWeather");
            if(localStorageWeather) {
                renderWeatherData();
        ...
        if((typeof RightPanel !== 'undefined') && (newTabData.financeData || newTabData.sportsData  || newTabData.aolData || extGlobal.constants.mapQuestUI)) { //for mapQuest we render even if data is not loaded yet - data is only geolocation for now
...
function RightPanel(newTabData, frValue, typeParam, css) {
    var viewUtils = new ViewUtils();
     function renderRightPanel() {
        var rightPanel = document.getElementById("rightPanel"),
            rpContent = document.createElement("div"),
            rightPanelDisplay =  extGlobal.browserGap.localStorage.getItem("rightPanelDisplay"),
            mainContainer = document.getElementById("mainContainer");

        rightPanel ? rightPanel.parentNode.removeChild(rightPanel) : null;
        rightPanel = document.createElement("div");
        rightPanel.setAttribute("id", "rightPanel");
        rpContent.setAttribute("id", "rpContent");
        rightPanel.appendChild(rpContent);
        mainContainer.parentNode.insertBefore(rightPanel, mainContainer.nextSibling);

        resizeRightPanelBlur();

        if (rightPanelDisplay && rightPanelDisplay === "collapse") {
            rightPanel.style.display = "none"; //after generating the blur we hide the right panel
            rightPanel.classList.add("hideOnRight");
            setTimeout(function() {
        ...
        if (newTabData.financeData && typeof FinanceRenderer !== "undefined") {
            renderFeaturePanelTopRight("finance");
            var financeRenderer = new FinanceRenderer(newTabData).renderFinance(frValue,typeParam);
        } else if (typeof MapQuestRenderer !== "undefined") {
            renderFeaturePanelTopRight("mapQuest");
            var mapQuestRenderer = new MapQuestRenderer(newTabData).renderMapQuest(frValue, typeParam);
        }
        if (newTabData.sportsData && typeof SportsRenderer !== "undefined") {
            renderFeaturePanelTopRight("sports");
            var sportsRenderer = new SportsRenderer(newTabData).renderSports(frValue,typeParam);
        }
        if ((newTabData.breakingNews || newTabData.trendingNowData) && typeof NewsRenderer !== "undefined") {
            renderFeaturePanelTopRight("news");
            var newsRenderer = new NewsRenderer(newTabData).renderNews(frValue, typeParam);
        }
        if (newTabData.aolData && typeof AolRenderer !== "undefined") {
            renderFeaturePanelTopRight("aol");
            var aolRenderer = new AolRenderer(newTabData).renderAol(); //TODO: may need to pass the frValue/typeParam equivalent for Aol search
        }
        ...
        if (rightPanelDisplay && rightPanelDisplay === "collapse") {
            toggleRightPanel(null, "pageLoad"); //after all the right panel was generated (with display=none), we toggle to get in the collapsed position
        }

        if (!extGlobal.constants.mapQuestUI) {
            renderFooter(frValue, typeParam);
        }
    }

    function toggleRightPanel(event, pageLoad) {

        function toggleAction () {
            viewUtils.resizeNewTab();
            var openCloseWrap = document.getElementById("hideShowPanel");
            var mainContainer = document.getElementById("mainContainer");
            var topSiteDiv = document.getElementById("topSites");
            var weatherContainer = document.getElementById("weatherInfo");
            var rightPanel = document.getElementById("rightPanel");
            rightPanel.classList.remove("translateBack"); //removing class after transition is over


            if (openCloseWrap.className.indexOf("collapseIconWrap") > -1) { //collapsing
                extGlobal.browserGap.localStorage.setItem("rightPanelDisplay", "collapse");
                mainContainer.classList.remove("withRightPanel");
                topSiteDiv.classList.remove("withRightPanel");

                openCloseWrap.classList.remove("collapseIconWrap");
                openCloseWrap.classList.add("expandIconWrap");
                openCloseWrap.childNodes[0].classList.remove("collapseIcon");
                openCloseWrap.childNodes[0].classList.add("expandIcon");
                document.body.appendChild(openCloseWrap);

                if (weatherContainer) {
                    weatherContainer.classList.remove("withRightPanel");
                }

            } else { //expanding
                extGlobal.browserGap.localStorage.setItem("rightPanelDisplay", "expand");
                mainContainer.classList.add("withRightPanel");
                topSiteDiv.classList.remove("withRightPanel");

                var topRightPanel = document.getElementById("topRightPanel");
                openCloseWrap.classList.remove("expandIconWrap");
                openCloseWrap.classList.add("collapseIconWrap");
                openCloseWrap.childNodes[0].classList.remove("expandIcon");
                openCloseWrap.childNodes[0].classList.add("collapseIcon");
                topRightPanel.insertBefore(openCloseWrap, topRightPanel.firstChild);

                if (weatherContainer) {
                    weatherContainer.classList.add("withRightPanel");
                }
            }
        }
        ...
        if (!pageLoad) {
            var rightPanel = document.getElementById("rightPanel");
            if (!rightPanel.classList.contains("hideOnRight")) {
                rightPanel.setAttribute("class", "hideOnRight");
            } else {
                rightPanel.setAttribute("class", "translateBack");
            }
            setTimeout(function() {
                toggleAction();
            }, 200);
            $("#rightPanelBlur").fadeToggle(200, resizeRightPanelBlur);


        } else { //triggered after page load because right panel is collapsed
            var rightPanelBlur = document.getElementById("rightPanelBlur");
            setTimeout(function() {
                toggleAction();
                rightPanelBlur.style.display = "none";
            }, 100);
        }
    }

    function renderFeaturePanelTopRight(panelType) {
        var container = document.getElementById("rpContent"),
            topRightPanel = document.createElement("div");

        topRightPanel.setAttribute("id", "topRightPanel");
        container.insertBefore(topRightPanel, container.childNodes[0]);

        var imageLogo = document.createElement("span");
        var financeExp = document.createElement("span");
        var sportsExp = document.createElement("span");
        var newsExp = document.createElement("span");
        var openClose = document.createElement("span");
        var openCloseWrap = document.createElement("span");

        if (panelType === "finance") {
            //financeExp.classList.add("selectedExp");
            imageLogo.setAttribute("class", "panelFinanceLogo");
            imageLogo.addEventListener("click", function() {
                if (extGlobal.constants.finance_default_region.toLowerCase() !== "us") {
                    window.location = extGlobal.constants.finance_property_url.replace("{region}", extGlobal.constants.finance_default_region.toLowerCase() + ".") + "?fr=" + frValue;
                } else {
                    window.location = extGlobal.constants.finance_property_url.replace("{region}", "") + "?fr=" + frValue;
                }
            });
        } else if (panelType === "mapQuest") {
            imageLogo.setAttribute("class", "mqIcon");
            imageLogo.addEventListener("click", function() {
                window.location = "https://www.mapquest.com/";
            });
        } else if (panelType === "sports") {
            sportsExp.classList.add("selectedExp");
            imageLogo.setAttribute("class", "panelSportsLogo");
            imageLogo.addEventListener("click", function () {
                window.location = "https://sports.yahoo.com/?fr=" + frValue;
            });
        } else if (panelType === "news") {
            newsExp.classList.add("selectedExp");
            imageLogo.setAttribute("class", "panelYahooLogo");
            imageLogo.addEventListener("click", function () {
                window.location = "https://news.yahoo.com/?fr=" + frValue;
            });
        } else if (panelType === "aol") {
            imageLogo.classList.add("aolLogo");
            //TODO: add the icon for AOL on the top of the right rail here
        }
        openClose.setAttribute("class", "collapseIcon");
        openCloseWrap.addEventListener("click", toggleRightPanel);
        openCloseWrap.setAttribute("class", "collapseIconWrap");
        openCloseWrap.setAttribute("id", "hideShowPanel");
        openCloseWrap.appendChild(openClose);


        //financeExp.textContent = extGlobal.browserGap.getLocalizedString("newtab_finance_label");
        //sportsExp.textContent = extGlobal.browserGap.getLocalizedString("newtab_sports_label");
        //newsExp.textContent = extGlobal.browserGap.getLocalizedString("newtab_news_label");
        //financeExp.classList.add("uiType");
        //sportsExp.classList.add("uiType");
        //newsExp.classList.add("uiType");

        function updateUI(uiExp) {
            extGlobal.browserGap.emitToMain({newTab: true, updateExp: uiExp});
            var rightPanel = document.getElementById("rightPanel");
            if (rightPanel) {
                var spinner = document.getElementById("rightPanelLoad");
                if (!spinner) {
                    spinner = document.createElement("div");
                }
                spinner.setAttribute("id", "rightPanelLoad");
                //putting the spinner in the center of the right panel
                spinner.style.top = ((rightPanel.offsetHeight + (rightPanel.offsetTop *2)) / 2) - 16 + "px";
                spinner.style.left = ((rightPanel.offsetWidth + (rightPanel.offsetLeft *2)) / 2) - 16 + "px";
                spinner.style.display = "block";
                rightPanel.parentNode.removeChild(rightPanel);
                document.body.appendChild(spinner);
            }

            if (!extGlobal.rightPanelListener) {
                extGlobal.rightPanelListener = true;
                chrome.runtime.onConnect.addListener(function(port) {
                    port.onMessage.addListener(function(msg) {
                        if (msg.type === "refreshUI") {
                            if (msg.financeData) {
                                newTabData.financeData = msg.financeData;
                                newTabData.sportsData = null;
                                //newTabData.breakingNews = null;
                                //newTabData.trendingNowData = null;
                            }
                            /*if (msg.sportsData && SportsRenderer) {
                                newTabData.sportsData = msg.sportsData;
                                newTabData.financeData = null;
                                newTabData.breakingNews = null;
                                newTabData.trendingNowData = null;
                            }*/
                            /*if ((msg.trendingNowData || msg.breakingNews) && NewsRenderer) {
                                newTabData.trendingNowData = msg.trendingNowData;
                                newTabData.breakingNews = msg.breakingNews;
                                newTabData.sportsData = null;
                                newTabData.financeData = null;
                            }*/
        ...
        /*financeExp.addEventListener("click", function() {
            if (financeExp.className.indexOf("selectedExp") === -1) {
                updateUI("financeUI");
            }
        });*/
        /*sportsExp.addEventListener("click", function() {
            if (sportsExp.className.indexOf("selectedExp") === -1) {
                updateUI("sportsUI");
            }
        });
        newsExp.addEventListener("click", function() {
            if (newsExp.className.indexOf("selectedExp") === -1) {
                updateUI("newsUI");
            }
        });*/

        //topRightPanel.appendChild(financeExp);
        //topRightPanel.appendChild(sportsExp);
        //topRightPanel.appendChild(newsExp);
        topRightPanel.appendChild(openCloseWrap);
        topRightPanel.appendChild(imageLogo);
    }

    function resizeRightPanelBlur(){
        //clip right rail
        var elmt = document.getElementById("rightPanel");
        if (!elmt.classList.contains("hideOnRight")) {
            var blur = document.getElementById("rightPanelBlur");
            var crdts = elmt.offsetLeft-(elmt.offsetWidth/6)-7; //-7 is for blur should be look like a layer behind the right rail
            var params = [0, screen.width, screen.height, crdts];
            var clip = { clip: "rect("+params.join("px,")+"px)" };
    ...
    function renderFooter(frCode, type) {
        ...
        if (!extGlobal.constants.aolUI) { //aol UI has a specific rendering in aolRenderer.js
            var footer = document.createElement("div"),
                rightPanel = document.getElementById("rightPanel"),
                yahoo = document.createElement("span"),
                tos = document.createElement("a"),
                policy = document.createElement("a"),
                tos_1 = document.createElement("span"),
                tos_2 = document.createElement("span"),
                policy_1 = document.createElement("span"),
                policy_2 = document.createElement("span"),
                feedback = document.createElement("span"),
                feedbackUrl = extGlobal.browserGap.getLocalizedString("newtab_extension_feedback_link"),
                feedbackLabel = extGlobal.browserGap.getLocalizedString("newtab_extension_feedback_link_label"),
                yahooUrl = extGlobal.browserGap.getLocalizedString("newtab_extension_homepage_url") + "?fr=" + frCode +"&type=" + type,
                yahooLabel = "Yahoo";
            ...
            footer.className = "rightPanelFooter";


            if (newTabData.sportsData && typeof SportsRenderer !== "undefined") {
                var sportsUrl = "https://sports.yahoo.com?fr=" + frCode + "&type=" + type;
                var sportsLabel = "Yahoo Sports - NBC Sports Network, Stats LLC, Opta. All Rights Reserved";
                yahoo.classList.add("block");
                yahoo.appendChild(createLink(sportsUrl, sportsLabel));
            } else {
                yahoo.appendChild(createLink(yahooUrl, yahooLabel));
            }
            feedback.appendChild(createLink(feedbackUrl, feedbackLabel));

            footer.appendChild(yahoo).parentNode.appendChild(tos).parentNode.appendChild(policy).parentNode.appendChild(feedback);
            rightPanel.appendChild(footer);
        }

    }

    this.renderRightPanel = renderRightPanel;
    this.resizeRightPanelBlur = resizeRightPanelBlur;
    return this;
}
...
function Constants() { //jshint ignore : line
    ...
    this.finance_news_url = "https://partner-query.finance.yahoo.com/v2/finance/quotefeeds?count=20&symbols={QUOTES}&lang={lang}&region={region}";
